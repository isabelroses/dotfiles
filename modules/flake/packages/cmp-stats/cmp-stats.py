# Vendored from nixpkgs ci/eval/compare/cmp-stats.py (MIT licensed).
# Source: https://github.com/NixOS/nixpkgs/blob/master/ci/eval/compare/cmp-stats.py
import argparse
import json
import numpy as np
import pandas as pd
...
[^envs.bytes]: Size in bytes of all `Env` objects allocated by the Nix evaluator.
[^list.bytes]: Size in bytes of all lists allocated by the Nix evaluator.
[^sets.bytes]: Size in bytes of all attrsets allocated by the Nix evaluator.
...
[^nrOpUpdates]: The number of attrset merge operations (`//`) performed.
[^nrPrimOpCalls]: The number of function calls to primops (Nix builtins) ever made.
[^nrThunks]: The number of thunks ever made.
...
def main():
    ...
    before_metrics = load_all_metrics(Path(options.before))
    after_metrics = load_all_metrics(Path(options.after))
    pairwise_test_results = perform_pairwise_tests(before_metrics, after_metrics)
    print(pairwise_test_results.to_markdown(explain=options.explain))
