...
class ThemeService extends Service {
	...
	setupWallpaper() {
		execAsync(["swww", "img", this.getSetting("wallpaper")]).catch(print);
	}
	...
	setSetting(prop, value) {
		...
		settings[prop] = value;
		writeFile(JSON.stringify(settings, null, 2), this._settingsPath).catch(
			print,
		);
...
export class Theme {
	...
	static reset() {
		Theme.instance.reset();
	}
	static openSettings() {
		Theme.instance.openSettings();
	}
	static getSetting(prop) {
		return Theme.instance.getSetting(prop);
	}
	static setSetting(prop, value) {
		return Theme.instance.setSetting(prop, value);
	}
}
...
export const Toggle = (props) =>
	Button({
		...
		onClicked: (btn) => {
			...
			if (index > btn._list.length) index = 0;
